        ...
        {!task ? (
          ...
          <>
            ...
            <div className="assign-field" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
              <div>
                <label>우선순위</label>
                <select className="inp" value={priority} onChange={(e) => setPriority(e.target.value as Priority)}>
              ...
              <div>
                <label>상태</label>
                <select className="inp" value={status} onChange={(e) => setStatus(e.target.value as TaskStatus)}>
            ...
            {(task.reportRequired || reportLink) && (
              <div className="assign-field">
                <label>📊 보고링크</label>
                <input className="inp" value={reportLink} onChange={(e) => setReportLink(e.target.value)} placeholder="https://..." />
              </div>
            )}
            {(task.videoRequired || videoLink) && (
              <div className="assign-field">
                <label>🎥 설명영상</label>
                <input className="inp" value={videoLink} onChange={(e) => setVideoLink(e.target.value)} placeholder="https://..." />
              </div>
            )}

            <div className="assign-field">
              <label>상세 설명 (메모)</label>
              <textarea className="inp" value={description} onChange={(e) => setDescription(e.target.value)} style={{ minHeight: 70 }} />
            </div>
            ...
            <div className="assign-field">
              ...
              <textarea
                ...
                placeholder="부여 시 생성된 업무설명 doc (없으면 메모 적고 AI 재생성)"
                style={{ minHeight: 140 }}
              />
            </div>
